...
/* Descending order */
gint g_record_descmp_by_length(const void *void_a,
							   const void *void_b,
							   gpointer user_data)
{
	struct containerRecord *a = a;
	struct containerRecord *b = b;
	return b->size - a->size;
}

gint g_record_cmp_by_id(const void *void_a,
						const void *void_b,
						gpointer user_data)
{
...
int rewrite_buffer_push(struct chunk *c)
{
	...
	if (CHECK_CHUNK(c, CHUNK_FILE_START) || CHECK_CHUNK(c, CHUNK_FILE_END) || CHECK_CHUNK(c, CHUNK_SEGMENT_START) || CHECK_CHUNK(c, CHUNK_SEGMENT_END))
		return 0;
...
struct chunk *rewrite_buffer_pop()
{
	...
	if (c && !CHECK_CHUNK(c, CHUNK_FILE_START) && !CHECK_CHUNK(c, CHUNK_FILE_END) && !CHECK_CHUNK(c, CHUNK_SEGMENT_START) && !CHECK_CHUNK(c, CHUNK_SEGMENT_END))
	{
		/* A normal chunk */
